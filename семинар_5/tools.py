...
# ===========================================================================
# 6. Сравнение метрики в двух периодах  (домашнее задание к Семинару 5)
# ===========================================================================

import calendar as _calendar

# какие метрики поддерживаем и как достаём из них число
_COMPARE_METRICS = {"key_rate", "fx_USD", "fx_EUR", "fx_CNY", "cpi", "unemployment"}


def _period_to_date(period: str) -> _date:
    """'YYYY-MM' -> последний день месяца; 'YYYY-MM-DD' -> сама дата."""
    period = period.strip()
    if re.fullmatch(r"\d{4}-\d{2}", period):
        y, m = int(period[:4]), int(period[5:7])
        if not (1 <= m <= 12):
            raise ValueError(f"месяц вне 1..12 в периоде {period!r}")
        last = _calendar.monthrange(y, m)[1]
        return _date(y, m, last)
    if re.fullmatch(r"\d{4}-\d{2}-\d{2}", period):
        return datetime.strptime(period, "%Y-%m-%d").date()
    raise ValueError(f"период {period!r} не в формате YYYY-MM или YYYY-MM-DD")


def _metric_value(metric: str, d: _date) -> dict:
    """Вернуть сырой ответ нужного инструмента для метрики на дату d.
    Достаём число под ключом, который зависит от метрики."""
    if metric == "key_rate":
        obs = get_key_rate(on_date=d.isoformat())
        return {"obs": obs, "value": obs.get("rate"), "date": obs.get("date", d.isoformat())}
    if metric.startswith("fx_"):
        cur = metric.split("_", 1)[1]
        obs = get_fx_rate(currency=cur, on_date=d.isoformat())
        return {"obs": obs, "value": obs.get("rate"), "date": obs.get("date", d.isoformat())}
    if metric == "cpi":
        obs = get_inflation(d.year, d.month)
        return {"obs": obs, "value": obs.get("cpi_yoy"), "date": f"{d.year}-{d.month:02d}"}
    if metric == "unemployment":
        obs = get_unemployment(d.year, d.month)
        return {"obs": obs, "value": obs.get("unemployment"), "date": f"{d.year}-{d.month:02d}"}
    raise ValueError(f"метрика {metric!r} не поддерживается")


def compare_periods(metric: str, period_a: str, period_b: str) -> dict:
    """
    Сравнить значение одной метрики в двух периодах.
    ...
    Логику курсов/ставок/инфляции/безработицы НЕ дублируем — зовём готовые
    get_fx_rate / get_key_rate / get_inflation / get_unemployment.
    Любую ошибку возвращаем как {"metric": ..., "error": ...}, не кидаем исключение.
    """
    if metric not in _COMPARE_METRICS:
        return {
            "metric": metric,
            "error": f"неизвестная метрика {metric!r}; допустимы: {sorted(_COMPARE_METRICS)}",
        }
    try:
        da = _period_to_date(period_a)
        db = _period_to_date(period_b)
    except ValueError as e:
        return {"metric": metric, "error": str(e)}

    a = _metric_value(metric, da)
    b = _metric_value(metric, db)

    if a["value"] is None:
        return {"metric": metric, "error": f"нет данных для периода A ({period_a}): {a['obs']}"}
    if b["value"] is None:
        return {"metric": metric, "error": f"нет данных для периода B ({period_b}): {b['obs']}"}

    va, vb = float(a["value"]), float(b["value"])
    ratio = (vb / va) if va != 0 else None

    src_a = a["obs"].get("source", "?")
    src_b = b["obs"].get("source", "?")
    source = src_a if src_a == src_b else f"{src_a}+{src_b}"

    return {
        "metric": metric,
        "a": {"date": a["date"], "value": round(va, 6)},
        "b": {"date": b["date"], "value": round(vb, 6)},
        "delta": round(vb - va, 6),

...
# ДЗ-2 — поток событий в JSONL. Открываем в режиме "a": логи копятся между
# запусками, файл читается как лента событий (по строке на шаг/вызов).
TRACE_PATH = Path(__file__).resolve().parent / "trace.jsonl"


def _log_event(run_id: str, entry: dict) -> None:
    """Дописать одну строку в trace.jsonl. ts — момент записи, run_id —
    общий для всех строк одного прогона (uuid4)."""
    record = {
        "run_id": run_id,
    ...
    try:
        ...
        pass  # лог не должен ронять агента
...
- compare_periods: сравнить ОДНУ метрику в двух периодах; сам возвращает delta
  (b−a) и ratio (b/a). metric ∈ {key_rate, fx_USD, fx_EUR, fx_CNY, cpi,
  unemployment}, периоды YYYY-MM или YYYY-MM-DD
...
7. Вопрос «во сколько раз / на сколько изменилась ОДНА метрика между двумя
   датами» решай одним вызовом compare_periods (он сам даёт delta и ratio);
   отдельный calculate тогда не нужен.
"""
...
def run_agent(
    ...
    *,
    ...
    run_id = uuid.uuid4().hex  # ДЗ-2 — общий id всех строк лога этого прогона
    ...
    for step in range(1, max_iter + 1):
        ...
        if not msg.tool_calls:
            ...
            _log_event(run_id, {"step": step, "final": msg.content})
            return _finish(
                {
                    "answer": msg.content,
                    "structured": None,
        ...
        if submit is not None:
            ...
            _log_event(run_id, {"step": step, "final": ans.answer})
            return _finish(
                {
                    "answer": ans.answer,
                    "structured": ans,
